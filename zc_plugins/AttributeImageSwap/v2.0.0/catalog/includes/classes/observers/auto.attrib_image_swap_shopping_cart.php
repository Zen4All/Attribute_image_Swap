    ...
    if (is_array($productArray)) {
      ...
      for ($i = 0, $n = count($productArray); $i < $n; $i++) {
        ...
        $sql = "SELECT pa.options_values_id, pa.options_id
                ...
                AND pa.products_id = " . (int)$productArray[$i]['id'] . "
        ...
        foreach ($productArray[$i]['attributes'] as $key => $value) {
          ...
          $swap_results = $db->Execute($sql_swap);

          if ($swap_results->RecordCount() == 0) {
          ...
          foreach ($swap_results as $swap_result) {
            //trigger_error('swap results: ' . print_r($swap_result, true), E_USER_WARNING);
            if (empty($swappable[$swap_result['options_id']])) {
              $swappable[$swap_result['options_id']] = $swap_result['options_values_id'];
            }
          }
