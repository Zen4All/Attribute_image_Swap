...
class zcObserverAttribImageSwap extends base
{
    ...
    protected function updateNotifyAttributesModuleStartOptionsLoop(&$callingClass, $notifier, $i, &$products_options_fields)
    {
        ...
        if (($this->products_options_names_fields['products_options_type'] == PRODUCTS_OPTIONS_TYPE_SELECT) || ( $this->products_options_names_fields['products_options_type'] == PRODUCTS_OPTIONS_TYPE_SELECT_SBA)) {
            $params = '';
...
//                case '6':
    ...
    public function zen_link($texts, $hrefs, $parameters = '')
    {
        ...
        for ($i = 0, $n = sizeof($texts); $i < $n; $i++) {
            ...
            if (zen_not_null($hrefs)) {
                $link .= 'href="' . zen_output_string($hrefs[$i]['text']) . '"';
            }
    ...
    public function get_attrib_image($products_id = 0, $products_options_values_id = 0)
    {
        ...
        foreach ($products_color_image as $item) {
            if ($item['language_id'] != $_SESSION['languages_id'] && $item['language_id'] != $default_lang_id) {
                continue;
            }
            if ($item['language_id'] == $_SESSION['languages_id']) {
                $products_image['lang'] = $item['attributes_image'];
                continue;
            }
            if ($item['language_id'] == $default_lang_id) {
                $products_image['default'] = $item['attributes_image'];
                continue;
            }
        }
